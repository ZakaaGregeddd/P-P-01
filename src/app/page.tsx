...
async function getLatestYoutubeVideos() {
  const fallbackVideos = [
    {
      id: 'XSSR9qoqCV8',
      title: 'Diculik Monster!! - R.E.P.O part 2',
      description: '45 x ditonton • 2 bulan yang lalu'
    },
    {
      id: '29dAczO4bWU',
      title: 'Nyari Barang Bekas - Repo part 1',
      description: '24 x ditonton • 3 bulan yang lalu'
    }
  ];

  try {
    const response = await fetch('https://www.youtube.com/@Zakaagreged/videos', {
    ...
    if (!match) return fallbackVideos;
    ...
    function findVideos(obj: any) {
      ...
      if (obj.lockupViewModel) {
        ...
        if (sources.length > 0 && sources[0].url) {
          const matchId = sources[0].url.match(/\/vi\/([a-zA-Z0-9_-]{11})\//);
        ...
        if (rows.length > 0 && rows[0].metadataParts) {
          desc = rows[0].metadataParts.map((p: any) => p.text?.content || '').join(' • ');
        }
        ...
        if (videoId && title) {
          extracted.push({
            id: videoId,
            title,
            description: desc || 'Video update dari channel YouTube.'
          });
        }
      }
    ...
    findVideos(json);
    return extracted.length >= 2 ? extracted.slice(0, 2) : fallbackVideos;
  } catch (error) {
    console.error('Error fetching dynamic YouTube videos:', error);
...
export default async function Home() {
  ...
  return (
    <div className="relative w-full overflow-hidden">
      ...
      <div id="hero-section" className="pt-16 pb-16 px-margin-mobile md:px-margin-desktop max-w-max-width mx-auto relative z-10 flex flex-col gap-16 scroll-mt-[-30px]">
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-gutter items-stretch min-h-[calc(100vh-160px)]">
          ...
          <div className="md:col-span-7 lg:col-span-8 flex flex-col justify-between gap-8 relative contents md:flex">
            ...
            <div className="flex flex-col gap-4 order-1 md:order-none">
              ...
              {(() => {
                ...
                return (
                  <div className="flex flex-wrap gap-4 mt-2 z-10 relative items-center">
            ...
            <div className="glass-panel p-5 relative overflow-hidden group hover:border-secondary/40 transition-all duration-300 order-4 md:order-none">
              {/* Section Header */}
          ...
          <div className="md:col-span-5 lg:col-span-4 relative mt-12 md:mt-0 flex flex-col justify-center order-2 md:order-none">
        ...
        <section className="flex flex-col gap-12">
          <div className="flex justify-between items-end border-b border-outline-variant/30 pb-4">
            ...
            <div className="font-technical-sm text-[10px] sm:text-technical-sm text-secondary">DATA.SET // 01-02</div>
          </div>
        ...
        <section className="flex flex-col gap-12 mt-8">
          <div className="flex justify-between items-end border-b border-outline-variant/30 pb-4">
            <h2 className="font-headline-md text-headline-md text-primary tracking-tight">FEATURED YOUTUBE CONTENT</h2>
            <div className="font-technical-sm text-[12px] sm:text-technical-sm text-secondary">CHANNEL // @Zakaagreged // Gregeddd</div>
          </div>
