...
export default function YouTubeGrid({ videos = [] }: { videos: VideoData[] }) {
  ...
  return (
    <div 
      ref={gridRef}
      className="grid grid-cols-1 md:grid-cols-2 gap-[70px] p-1 max-w-[990px] mx-auto w-full"
    >
      {videos.map((video, idx) => {
        ...
        return (
          <div 
            key={video.id}
            data-id={dataId}
            className={`glass-panel p-4 relative group glow-hover hover:border-secondary/40 transition-all duration-[800ms] ease-out transform flex flex-col gap-3 ${
